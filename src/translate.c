...
u16 predicate_index(const string* str, const struct predicate* predicates, const uint count) {
...
static void convert_types_to_facts(state* s, const uint type_count, const string* type_parents,
                                   const string* types, const uint object_count,
...
static void convert_facts(state* s, const struct fact* facts, const uint len,
                          const struct predicate* predicates, const uint predicate_count,
                          const string* objects, const uint object_count) {
        u16 args[16];
...
// Converts an expressions into multiple expressions which consist only of a
// conjuction of atoms I.e. Remove or which might lead to multiple expressions
static uint flatten_expression(struct expression* exps, const struct expression* exp) {
...
static uint convert_expression(atom_t* atoms, const struct expression* exp, const string* vars,
                               const uint var_count, const struct predicate* predicates,
        ...
        while (queue_len) {
                ...
                case E_AND:
                case E_OR:
                        fprintf(stderr, "Internal error: convert_expression "
                                        "called with non flat "
                                        "expression\n");
                        exit(1);
                }
        }
...
// Each sub-expression will lead to a differing scheme, and even more in the
// case of multiple OR's
static void convert_action(schema_t* schemas, uint* count, const struct action* action,
                           const struct predicate* predicates, const uint predicate_count) {
        ...
        for (uint i = 0; i < pre_exp_count; i++) {
                ...
                uint   pre_atom_count =
                    convert_expression(pre_atoms, &pre_exps[i], action->vars, action->var_count,
                                       predicates, predicate_count);
                for (uint t = 0; t < eff_exp_count; t++) {
                        atom_t eff_atoms[MAX_ATOMS];
                        uint   eff_atom_count =
                            convert_expression(eff_atoms, &eff_exps[t], action->vars,
                                               action->var_count, predicates, predicate_count);
        ...
        INFO("Action %.*s translated to %d schemes", action->name.len, action->name.ptr,
             pre_exp_count * eff_exp_count);
}

task_t translate(const struct domain* domain, const struct problem* problem) {
        task_t task = {0};
        uint   offset;
        ...
        convert_types_to_facts(task.init, domain->type_count, domain->type_parents, domain->types,
                               problem->object_count, problem->object_types);
        TRACE("Translate init");
        convert_facts(task.init, problem->inits, problem->init_count, domain->predicates,
                      domain->predicate_count, problem->objects, problem->object_count);
        TRACE("Translate goal");
        convert_facts(task.goal, problem->goals, problem->goal_count, domain->predicates,
                      domain->predicate_count, problem->objects, problem->object_count);
        TRACE("Translate actions");
        for (uint i = 0; i < domain->action_count; i++) {
                TRACE("Translate action %.*s", domain->actions[i].name.len,
                      domain->actions[i].name.ptr);
                convert_action(task.schemas, &task.schema_count, &domain->actions[i],
