...
path_t solve(const state* init, const state* goal) {
        path_t      path             = {0};
        u64         states_generated = 0;
        statespace* ss               = statespace_new();
        nodeval*    queue            = NULL;

        nodeval start = {.s = state_clone(init), .val = eval(init)};
        ...
        while (arrlenu(queue) > 0) {
                ...
                while (expand_step(node, &action, args, &child)) {
                        ...
                        nodeval nval = {.s = child, .val = eval(child)};
                        statespace_add(ss, child);
                        arrpush(queue, nval);
                        if (statespace_count(ss) % 1000 == 0) {
                                INFO("Generated/Count: %zu/%zu", states_generated,
                                     statespace_count(ss));
                        }
                }
        }
