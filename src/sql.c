...
char* sql_insert(const string* name, uint vars) {
        ...
        sprintf(buf, "INSERT INTO \"%.*s\" (%s) VALUES (%s);", name->len, name->ptr, buf_params,
                buf_values);
        return buf;
}

char* sql_action(const string* preds, const schema_t* schema) {
        ...
        for (uint i = 0; i < schema->pre_count; i++) {
                ...
                for (uint t = 0; t < atom->arg_count; t++) {
                        offset_joins += sprintf(buf_joins + offset_joins, " o%d.arg0 = t%d.arg%d",
                                                atom->args[t], i, t);
        ...
        for (uint i = 0; i < schema->pre_count; i++) {
                ...
                if (atom->arg_count != 0)
                        continue;
                offset_joins +=
                    sprintf(buf_joins + offset_joins, "\nWHERE EXISTS (SELECT * FROM \"%.*s\")",
                            preds[atom->predicate].len, preds[atom->predicate].ptr);
        }
