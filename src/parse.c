...
enum kind { EOI, LPAREN, RPAREN, DASH, ID, MAX_KIND };
...
const char* KIND_NAMES[MAX_KIND + 1] = {"EOI", "LPAREN", "RPAREN", "ID", "UNDEFINED"};

const char* KEYWORD_NAMES[MAX_KEYWORD + 1] = {
    "KEYWORD_DEFINE",       "KEYWORD_NAME",       "KEYWORD_DOMAIN",
    "KEYWORD_REQUIREMENTS", "KEYWORD_TYPES",      "KEYWORD_PREDICATES",
    "KEYWORD_ACTION",       "KEYWORD_PARAMETERS", "KEYWORD_PRECONDITION",
    "KEYWORD_EFFECT",       "KEYWORD_AND",        "KEYWORD_OR",
    "KEYWORD_NOT",          "KEYWORD_OBJECTS",    "KEYWORD_INIT",
    "KEYWORD_GOAL",         "UNDEFINED"};
...
void lexer_expect(lexer* l, enum kind kind) {
        enum kind actual;
        string    tmp;
...
void lexer_expect_def(lexer* l, enum keyword kw) {
        enum kind kind;
        string    tmp;
...
static void parse_id(lexer* l, string* id) {
        lexer_next(l, id);
        lexer_expect(l, RPAREN);
}

static void parse_ids(lexer* l, string* ids, uint* count) {
        *count = 0;
...
static void parse_expression(lexer* l, struct expression** exp) {
        string tmp;
        if (lexer_next(l, &tmp) == RPAREN)
                return;
...
static void parse_action(lexer* l, struct action* action) {
        ...
        while ((lexer_next(l, &tmp)) != RPAREN) {
                ...
                TRACE("Parse %s of action %.*s", KEYWORD_NAMES[keyword], action->name.len,
                      action->name.ptr);
                lexer_expect(l, LPAREN);
                switch (keyword) {
                case KEYWORD_PARAMETERS:
                        parse_typed(l, &action->var_count, action->vars, action->var_types, false);
                        break;
                case KEYWORD_PRECONDITION:
                        parse_expression(l, &action->precondition);
                        break;
                case KEYWORD_EFFECT:
                        parse_expression(l, &action->effect);
                        break;
                default:
...
static void parse_goal(lexer* l, struct fact* facts, uint* count) {
        lexer_expect(l, LPAREN);
