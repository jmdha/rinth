...
static Atom TranslateAtom(
    const char **v_predicates, char **vars, uint count, Expression *expression, bool val
) {
    int predicate = -1;
    for (uint i = 0; i < arrlenu(v_predicates); i++)
        if (strcmp(v_predicates[i], expression->data.atom.predicate) == 0) predicate = i;
    if (predicate == -1) {
        ERROR("Action has undeclared predicate %s", expression->data.atom.predicate);
        exit(1);
    }
...
static Atom *
TranslateExpression(const char **v_predicates, char **vars, uint count, Expression *expression) {
    typedef struct {
    ...
    while (arrlenu(v_queue) > 0) {
        ...
        case E_ATOM:
            arrpush(v_atoms, TranslateAtom(v_predicates, vars, count, e.expression, e.val));
            break;
        case E_AND:
...
Scheme TranslateAction(const char **v_predicates, Action *action) {
    Scheme scheme = {
        .name  = action->name,
        .v_pre = TranslateExpression(
            v_predicates, action->vars, action->var_count, action->precondition
        ),
        .v_eff = TranslateExpression(
            v_predicates, action->vars, action->var_count, action->effect //
        ),
    };
    action->name = NULL;
    return scheme;
